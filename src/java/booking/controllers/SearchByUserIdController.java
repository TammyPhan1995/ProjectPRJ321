...
public class SearchByUserIdController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            List<BookingDetailDTO> result = dao.findByUserId(username);
            request.setAttribute("INFO", result);
        } catch (Exception e) {
            log("ERROR AT SearchByUserIdController: " +e.getMessage());
        }finally{
            request.getRequestDispatcher("user/showBooking.jsp").forward(request, response);
        }
    }
