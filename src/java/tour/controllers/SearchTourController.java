...
public class SearchTourController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            List<TourDTO> result = dao.findByLineName(search);
            request.setAttribute("INFO", result);
        } catch (Exception e) {
            log("ERROR AT SearchTourController: " + e.getMessage());
            if(e.getMessage()==null){
                try {
                    String searchFromUpdate = (String) request.getAttribute("SEARCH");
                    TourDAO dao = new TourDAO();
                    List<TourDTO> result = dao.findByLineName(searchFromUpdate);
                    request.setAttribute("INFO", result);
                } catch (Exception ex) {
            ...
            request.getRequestDispatcher("admin/tour_management.jsp").forward(request, response);
        }
    }
